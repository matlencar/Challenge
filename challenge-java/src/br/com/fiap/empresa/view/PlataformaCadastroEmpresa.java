...
public class PlataformaCadastroEmpresa {
	...
	public static void main(String[] args) {
		...
		PropostaEmpresa proposta = new PropostaEmpresa(1500.00, " Sulamerica"," CLT", 2, " Vale Alimentação,"
				+ "Vale refeição, Day off, Curso de ingles, plano de carreira.");
		
		//Intanciando o cargo
		Cargo cargo = new Cargo("Estagio em Java e React", "O estagiario vai auxiliar na manutenção de softwares"
				+ ", e no suporte de criação de landing pages.");
		
		//Instanciando a vaga
		Vaga vaga = new Vaga("Não é necessario experiencia para a vaga, apenas bons conhecimemntos em logica,"
				+ "Basico de HTML e CSS, se tiver conhecimento em Java e React será um diferencial.");
		
		//Lendo as informações da empresa
		...
		//Lendo informações do endereço da empresa
		...
		System.out.println("Digite o dia da criação da empresa: ");
		int criacaoDia = leitor.nextInt();
		empresa.setCricaoDia(criacaoDia);
		
		System.out.println("Agora coloque o mes da criação da empresa: ");
		int criacaoMes = leitor.nextInt();
		empresa.setCricaoMes(criacaoMes);
		
		System.out.println("Por fim insira o ano da criação da empresa: ");
		int cricaoAno = leitor.nextInt();
		empresa.setCricaoAno(cricaoAno);
		
		//Lendo as informações de contato da empresa
		...
		//Lendo informações sobre o cargo
		System.out.println("Qual mentalidade de estagiario minha empresa vai precisar: ");
		String comportamental = leitor.next();
		cargo.setComportamental(comportamental);
		
		System.out.println("Qual qualidade pessoal minha empresa vai precisar: ");
		String pessoal = leitor.next();
		cargo.setPessoal(pessoal);
		
		System.out.println("Que profissional minha empresa vai precisar: ");
...
//		cargo.setNomeDoCargo(nomeDoCargo);
		...
		//Exibindo as informações da empresa cadastrada na plataforma
		...
		System.out.println("Data da criação da empresa: " + 
		...
		System.out.println("Cargo que está sendo oferecido "+cargo.getNomeDoCargo());
		System.out.println("O que vai excercer na sua função: "+cargo.getFuncao());
		...
		System.out.println("Tipo de contratação para o estagiario"+proposta.getContratacao());
		System.out.println("Tempo de contratação oferecido de no maximo: "+proposta.getTempo()+" anos.");
		System.out.println("Os demais beneficos que oferecemos na bolsa são: "+proposta.getBeneficios());
		System.out.println("Os requisitos para a vaga em questão: "+vaga.getRequisitos());
